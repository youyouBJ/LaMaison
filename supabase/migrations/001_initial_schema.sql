...
-- DROP TRIGGER IF EXISTS, WHERE NOT EXISTS sur les seeds.
...
-- 1.1 Trigger pour horodatage updated_at automatique
...
-- 1.2 Fonctions RLS (SECURITY DEFINER)
-- Ces fonctions accèdent à public.users en bypassant le RLS (via SECURITY DEFINER),
-- ce qui évite toute récursion infinie dans les politiques de la table users.
...
-- id = auth.users.id (pas d'auto-génération, l'admin insère manuellement après création Auth)
...
CREATE TABLE IF NOT EXISTS public.guests (
  ...
  first_name       text,
  last_name        text,
  email            text,           -- non obligatoire
  phone            text,           -- identifiant métier principal
  birthday         date,           -- non obligatoire
  notes            text,
...
-- Le RLS (section 7) restreint ensuite l'accès aux lignes autorisées.
-- =============================================
...
-- =============================================
-- SECTION 6 : ACTIVATION ROW LEVEL SECURITY
-- =============================================
...
-- =============================================
-- SECTION 7 : POLITIQUES RLS
-- Toutes les fonctions helper sont SECURITY DEFINER.
-- Elles ne déclenchent pas de récursion RLS sur la table users.
-- Convention : DROP IF EXISTS → CREATE (idempotent).
-- =============================================

-- ─── users ────────────────────────────────────────────────────────────────────
-- SELECT : son propre profil OU les membres du même restaurant (admin/manager)
...
-- INSERT : admin uniquement, dans son restaurant
...
-- UPDATE : admin uniquement, dans son restaurant
...
-- DELETE : admin uniquement, dans son restaurant
...
-- Tous les rôles staff peuvent créer/modifier des réservations
...
BEGIN
  ...
  -- Convention : 0=dimanche 1=lundi 2=mardi 3=mercredi 4=jeudi 5=vendredi 6=samedi
